...
func Test_EventsDependencies(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedLogs: []string{
				"Event failed due to incompatible probe\",\"event\":\"incompatible_probe_test",
			},
		...
		},
		{
			...
			unexpectedEvents: []events.ID{events.IncompatibleProbeTest, events.EventWithFailedDependency},
