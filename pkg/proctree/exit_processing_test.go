	...
	for _, test := range straitTreeTests {
		t.Run(
			test.name, func(t *testing.T) {
				...
				assert.False(
					t,
					proc.isAlive(types.Timestamp(exitEvent.Timestamp)),
					"Fork - %d, exit - %d",
					proc.getForkTime(),
					proc.getExitTime(),
				)
	...
	for _, test := range multiChildrenTests {
		t.Run(
			test.name, func(t *testing.T) {
				...
				require.NoError(t, err)
				assert.Equal(t, true, pp.isAlive(types.Timestamp(exitEvent.Timestamp)))
		...
		t.Run(
			tc.name, func(t *testing.T) {
				...
				for _, index := range tc.exitedThreadsIndexes {
					...
					require.True(t, ok)
					assert.Equal(t, shouldBeAlive, thread.isAlive(math.MaxInt))
...
func countChildTreeNodes(p *processNode) int {
	c := 0
	for _, chld := range p.getChildren() {
