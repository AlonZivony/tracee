...
func TestProcessTree_ProcessEvent(t *testing.T) {
	...
	processName := "ls"
	userId := 1337
	lastTimestamp := types.Timestamp(1639044471928009089)
	...
	checkProcessForksInfo := func(t *testing.T, p *processNode, forkTime int, exitTime int) {
		assert.Equal(t, types.Timestamp(forkTime), p.getForkTime())
		thread, ok := p.getThread(pid)
		require.True(t, ok, "thread", pid)
		assert.Contains(t, p.getThreads(), thread)
		assert.Equal(t, types.Timestamp(exitTime), thread.getExitTime())
		assert.Equal(t, types.Timestamp(forkTime), thread.getForkTime())
	}
	checkMissingProcessForkInfo := func(t *testing.T, p *processNode) {
		assert.Equal(t, types.Timestamp(0), p.getForkTime())
		thread, ok := p.getThread(pid)
		require.True(t, ok, "thread", pid)
		assert.Equal(t, types.Timestamp(0), thread.getExitTime())
		assert.Equal(t, types.Timestamp(0), thread.getForkTime())
	}
	...
	checkExecInfo := func(t *testing.T, p *processNode) {
		...
		assert.Equal(t, types.Timestamp(execEvent.Timestamp), p.getExecTime(lastTimestamp))
	...
	checkNotExecedInfo := func(t *testing.T, p *processNode) {
		...
		assert.Equal(t, uint(0), execInfo.ExecutionBinary.Ctime)
	...
	checkProcessExitInfo := func(t *testing.T, p *processNode) {
		thread, ok := p.getThread(pid)
		require.True(t, ok, "thread", pid)
		assert.Equal(t, types.Timestamp(processExitEvent.Timestamp), thread.getExitTime())
		assert.True(t, thread.isAlive(types.Timestamp(processForkEvent.Timestamp)))
		assert.False(t, thread.isAlive(types.Timestamp(processExitEvent.Timestamp)))
	}
	checkThreadExitInfo := func(t *testing.T, p *processNode) {
		thread, ok := p.getThread(tid)
		require.True(t, ok, "thread", tid)
		assert.Equal(t, types.Timestamp(threadExitEvent.Timestamp), thread.getExitTime())
		assert.True(t, thread.isAlive(types.Timestamp(threadForkEvent.Timestamp)))
		assert.False(t, thread.isAlive(types.Timestamp(threadExitEvent.Timestamp)))
	}
		...
		"Ordered flows", func(t *testing.T) {
			...
			t.Run(
				"Ordered main thread exit first", func(t *testing.T) {
					...
					assert.Equal(
						t,
						types.Timestamp(modifiedProcessExitEvent.Timestamp),
					...
					assert.Equal(
						t,
						types.Timestamp(modifiedThreadExitEvent.Timestamp),
		...
		"Missing event flow", func(t *testing.T) {
			...
			t.Run(
				"Missing thread exit", func(t *testing.T) {
					...
					forkedThread, ok := process.getThread(tid)
					require.True(t, ok, "thread", tid)
					assert.Equal(t, types.Timestamp(processExitEvent.Timestamp), forkedThread.getExitTime())
					checkProcessExitSuccess(t, tree)
				},
			)
			t.Run(
				"Missing main thread exit", func(t *testing.T) {
					...
					mainThread, ok := process.getThread(pid)
					require.True(t, ok, "thread", pid)
					assert.Equal(t, types.Timestamp(0), mainThread.getExitTime())
					tree.emptyDeadProcessesCache()
					process, err = tree.getProcess(pid)
					assert.NoError(t, err)
