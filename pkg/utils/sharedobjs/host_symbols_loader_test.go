...
var (
	openSymbol    = DynamicSymbol{name: "open", symbolType: elf.STT_FUNC}
	closeSymbol   = DynamicSymbol{name: "close", symbolType: elf.STT_FUNC}
	syscallSymbol = DynamicSymbol{name: "syscall", symbolType: elf.STT_FUNC}
)

var testDynamicSymbols = &dynamicSymbols{
	Exported: map[string]DynamicSymbol{
		openSymbol.name:  openSymbol,
		closeSymbol.name: closeSymbol,
	},
	Imported: map[string]DynamicSymbol{syscallSymbol.name: syscallSymbol},
}
...
func TestHostSharedObjectSymbolsLoader_GetDynamicSymbols(t *testing.T) {
	...
	t.Run(
		"Happy flow", func(t *testing.T) {
			...
			assert.Equal(
				t,
				fmt.Sprint(map[string]DynamicSymbol{openSymbol.name: openSymbol, "close": closeSymbol, "syscall": syscallSymbol}),
				fmt.Sprint(syms),
			)
		},
	)
...
func TestHostSharedObjectSymbolsLoader_GetExportedSymbols(t *testing.T) {
	...
	t.Run(
		"Happy flow", func(t *testing.T) {
			...
			assert.Equal(
				t,
				fmt.Sprint(map[string]DynamicSymbol{openSymbol.name: openSymbol, closeSymbol.name: closeSymbol}),
				fmt.Sprint(syms),
			)
		},
	)
...
func TestHostSharedObjectSymbolsLoader_GetImportedSymbols(t *testing.T) {
	...
	t.Run(
		"Happy flow", func(t *testing.T) {
			...
			assert.NoError(t, err)
			assert.Equal(t, fmt.Sprint(map[string]DynamicSymbol{syscallSymbol.name: syscallSymbol}), fmt.Sprint(syms))
		},
	)
...
func TestHostSharedObjectSymbolsLoader_loadSOSymbols(t *testing.T) {
	...
	t.Run(
		"Uncached non existing SO", func(t *testing.T) {
			t.Parallel()
	...
	t.Run(
		"Uncached existing SO", func(t *testing.T) {
			t.Parallel()
	...
	testCases := []struct {
		...
		},
		{
			...
			ExpecteResult: dynamicSymbols{
				Exported: make(map[string]DynamicSymbol),
				Imported: map[string]DynamicSymbol{
					"cap_to_text": {name: "cap_to_text", symbolType: elf.STT_FUNC},
				},
			},
		},
		{
			...
			ExpecteResult: dynamicSymbols{
				Exported: map[string]DynamicSymbol{
					"_obstack_memory_used": {name: "_obstack_memory_used", symbolType: elf.STT_FUNC},
				},
		...
		},
		{
			...
			Input: []elf.Symbol{
				...
				{Name: "cap_to_text", Info: byte(elf.STT_SECTION), Section: elf.SHN_UNDEF},
				{
					Name:    "_obstack_memory_used",
					Info:    byte(elf.STT_FUNC),
					Section: elf.SHN_UNDEF + 12,
			...
			ExpecteResult: dynamicSymbols{
				...
				Imported: map[string]DynamicSymbol{
					"__ctype_toupper_loc": {name: "__ctype_toupper_loc", symbolType: elf.STT_FUNC},
					"cap_to_text":         {name: "cap_to_text", symbolType: elf.STT_SECTION},
				},
			},
		},
	}
	...
	for _, testCase := range testCases {
		...
		t.Run(
			testCase.Name, func(t *testing.T) {
				...
				assert.Equal(
					t,
					fmt.Sprint(testCase.ExpecteResult.Imported),
					fmt.Sprint(dynamicSymbols.Imported),
				)
				assert.Equal(
					t,
					fmt.Sprint(testCase.ExpecteResult.Exported),
					fmt.Sprint(dynamicSymbols.Exported),
				)
			},
		)
	}
