...
func (cLoader *ContainersSymbolsLoader) GetDynamicSymbols(soInfo ObjInfo) (map[string]DynamicSymbol, error) {
	var err error
...
func (cLoader *ContainersSymbolsLoader) GetExportedSymbols(soInfo ObjInfo) (map[string]DynamicSymbol, error) {
	var err error
	soInfo.Path, err = cLoader.pathResolver.GetHostAbsPath(soInfo.Path, soInfo.MountNS)
	if err != nil {
		return nil, errfmt.WrapError(err)
...
func (cLoader *ContainersSymbolsLoader) GetImportedSymbols(soInfo ObjInfo) (map[string]DynamicSymbol, error) {
	var err error
	soInfo.Path, err = cLoader.pathResolver.GetHostAbsPath(soInfo.Path, soInfo.MountNS)
	if err != nil {
		return nil, errfmt.WrapError(err)
